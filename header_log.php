<!DOCTYPE html>
<html>
<head lang="en">
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>My Chop Book</title>
    <!--chop theme-->

    <!-- Latest compiled and minified CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.4/css/bootstrap.min.css">
    <!-- Optional theme -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.4/css/bootstrap-theme.min.css">
        <link rel="stylesheet" href="css/herostyle.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script>


</head>
<body>
<header>
	<div class="topnav">
	<div class="container">
		<div class="row">
			<div class="col-xs-12 col-sm-6">
            <a href="#link1"><img src="images/social1.png" alt="pinterest" height="25px"/></a>
            <a href="#link1"><img src="images/social2.png" alt="twitter" height="25px"/></a>
            <a href="#link1"><img src="images/social3.png" alt="facebook" height="25px"/></a>
            <a href="#link1"><img src="images/social4.png" alt="instagram" height="25px"/></a>
            </div>
            <div class="col-xs-12 col-sm-6 toplinks"><a href="#link1">Welcome, User / Log Out</a> | <a href="#link1">My Account</a> | <a href="mychops.php">MyChops</a></div>
		</div>
	</div><!-- end container-->
    </div><!-- end topnav-->
    
    <div class="mainnav">
	<div class="container">
		<div class="row">
            <div class="col-xs-12 col-sm-4 mainlinks">
            <a href="#link1">Recipes</a>
            <a href="feeds.php">Chop Feed</a>
            <a href="#link1">Chop Videos</a>
            </div>
            <div class="col-xs-12 col-sm-4 headerlogo">
            <a href="index.php"><img src="images/mychopbooklogo.png" alt="instagram"/></a>
            </div>
            <div class="col-xs-12 col-sm-4 topsearchbar">
        
            <form action="search.php"><input class="topsearchinput" name="search" placeholder="Search"></form></div>
		</div>
	</div><!-- end container-->
	</div><!-- end navarea-->
</header>